...
public class StudentRestImpl implements StudentRest {
    ...
    public ResponseEntity<?> list(final String student) {
        ...
        return ResponseHandler
                .response(STUDENT_MAPPER.studentsToRest(students));
    }
    ...
    @Override
    public ResponseEntity<?> save(StudentsRequestRest requestRest) {

        if (requestRest != null && requestRest.getStudents() != null
                && !requestRest.getStudents().isEmpty()) {
            List<Student> list = studentService.save(
                    STUDENT_MAPPER.restToStudents(requestRest.getStudents()));
            return ResponseHandler
                    .response(STUDENT_MAPPER.studentsToRest(list));
        }
        return ResponseHandler.response(HttpStatus.INTERNAL_SERVER_ERROR);
